...
Other statuses that appear in `openapi.yaml` — for example 201/202 on the point-of-sale
write operations, which are marked `x-verified: false` in the spec — have not been
observed against the live API. If you hit one that isn't in the table above, treat its
body as unconfirmed rather than assuming it matches the spec, and consider it worth a
report to Flyfish Europe through DealerWeb.

## Errors that look like success

Some responses carry HTTP 200 (or, for one SDK, a resolved Promise) while still meaning
"this didn't work the way you expected." Watch for these:
...
- **PHP SDK: `No data from API`.** The same `Exception` is thrown, with this message
  instead, for any other non-200 status or when the underlying curl request itself fails
  (a network problem, not an HTTP response at all). If curl reported its own error text,
